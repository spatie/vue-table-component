...
## Rendering footer rows

It might be useful to have a row to summarize data. To add a row into a table footer (i.e. [tfoot](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/tfoot)),
you will need to add a special row object as part of your `data` array. Currently, the data for that object will look
like any other row but you'll need to add the flag `isFooterRow` and set its value to `true` to have it rendered
in the footer. NOTE: alignment via column names will still be referenced from the existing `<table-column>`, so to add meaningful
text/labels to the footer cells, you'll need to use existing column names. In the example below, we use `firstName` to print out
...
<table-component
     ...
     { firstName: 'George', birthday: '25/02/1943', songs: 22 },
     { firstName: 'Ringo', birthday: '07/07/1940', songs: 2 },
     { firstName: 'Total Songs', songs: 166, isFooterRow: true },
     ]"
     sort-by="songs"
     sort-order="asc"
     >
